...
void InitDMA(void)
{
	...
	/* See DMA_interface.h and DMA_interface.c for definitions */
		...
		DMAInitialize();
  		...
  		DMASource = &AdcResult.ADCRESULT0;    //Point DMA source to ADC result register base
  		DMACH1AddrConfig(DMADest,DMASource);
  		//Defines burst, which transfers 1 word (2-1), increments source address by 1 (moves ADCRESULT0 to ADCRESULT1...), and increases source address by 2
  		DMACH1BurstConfig(2,1,2);
  		//Defines DMA transfer cycle, takes 1 burst, rest is ignored
  		DMACH1TransferConfig(1,1,0);
  		// Does bascially nothing
  		DMACH1WrapConfig(1,0,0,1);
  		// ADC1 interrupt latches DMA (DMA_SEQ1INT)
...
/* These functions are from examples */
